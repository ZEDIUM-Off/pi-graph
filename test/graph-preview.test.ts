...
test("preview includes markdown risks and mermaid", async () => {
  const graph = normalizeGraphConfig(await readJsonFile("graphs/review-loop.json"));
...
test("mermaid renders START and END", async () => {
  const graph = normalizeGraphConfig(await readJsonFile("test/fixtures/graphs/valid-simple.json"));
